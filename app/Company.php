...
class Company extends Model
{
     ...
     * @return int
     */
    public static function getTotal($page = 1)
    {
        $total = array();
        if(File::exists(self::getStoragePath())) {
            ...
            $limitBefore = $page <= 1 || $page == null ? 0 : ($page - 1) * 15;
            unset($reader);
        }
        if (is_array($total)) {
            $total[0]['total_page'] = $total_page;
            $total[0]['total_data_all'] = $total;
            $total[0]['total_data'] = count($total);
        }
    ...
    public static function getBatch($page){
        $test = 5;
        $batch = 1;
        $flag = false;
        while(!$flag){
